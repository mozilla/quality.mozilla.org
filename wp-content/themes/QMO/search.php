      ...
      <div class="event-date compact">
        ...
        <?php // Not all day, but the time spans more than one date (e.g., runs past midnight)
          elseif (!get_post_meta( $post->ID, '_EventAllDay' ) && (the_event_start_date($post->ID, false, $date_format) < the_event_end_date($post->ID, false, $date_format)) ) : ?>
            <abbr class="dtstart" title="<?php echo the_event_start_date( $post->ID, false, 'Y-m-d\TH:i:s' ); ?>"><?php echo the_event_start_date( $post->ID, false, $date_format ).", ".the_event_start_date( $post->ID, false, $time_format ); ?></abbr>
            &ndash;
            <abbr class="dtend" title="<?php echo the_event_end_date( $post->ID, false, 'Y-m-d\TH:i:s' ); ?>"><?php echo the_event_end_date( $post->ID, false, $date_format ).", ".the_event_end_date( $post->ID, false, $time_format ); ?></abbr>
...
<?php else : // if there are no posts ?>
  ...
  <h1 class="page-title">Sorry, we didn&#8217;t find anything for &ldquo;<?php the_search_query(); ?>&rdquo;</h1>
