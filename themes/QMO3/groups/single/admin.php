...
<form action="<?php bp_group_admin_form_action() ?>" name="group-settings-form" id="group-settings-form" class="standard-form full" method="post" enctype="multipart/form-data">
...
<?php if ( bp_is_group_admin_screen( 'edit-details' ) ) : ?>
  ...
  <p>
    <label for="group-name">* <?php _e( 'Team Name', 'qmo' ) ?></label>
    <input type="text" name="group-name" id="group-name" value="<?php bp_group_name() ?>" />
  </p>
  <p>
    <label for="group-desc">* <?php _e( 'Team Description', 'buddypress' ) ?></label>
    <textarea name="group-desc" id="group-desc" rows="6" cols="60"><?php bp_group_description_editable() ?></textarea>
  </p>
  ...
  <p><button type="submit" id="save" name="save"><?php _e( 'Save Changes', 'buddypress' ); ?></button></p>
...
<?php if ( bp_is_group_admin_screen( 'group-settings' ) ) : ?>
  ...
  <p class="submit"><button type="submit" id="save" name="save"><?php _e( 'Save Changes', 'buddypress' ); ?></button></p>
...
<?php if ( bp_is_group_admin_screen( 'group-avatar' ) ) : ?>
  ...
  <?php if ( 'upload-image' == bp_get_avatar_admin_step() ) : ?>
      ...
      <div class="generic-button" id="delete-avatar">
        <p><?php _e( "If you'd like to remove the existing avatar but not upload a new one, please use the delete avatar button.", 'buddypress' ) ?>
        <br><a class="delete" href="<?php bp_group_avatar_delete_link() ?>" title="<?php _e( 'Delete Avatar', 'buddypress' ) ?>"><?php _e( 'Delete Avatar', 'buddypress' ) ?></a></p>
      </div>
      <?php endif; ?>
    ...
    <?php if ( bp_group_has_members( 'per_page=15&exclude_banned=false' ) ) : ?>
      ...
      <?php if ( bp_group_member_needs_pagination() ) : ?>
        ...
        <div class="pagination no-ajax">
          ...
          <p id="member-count" class="pag-count">
            <?php bp_group_member_pagination_count() ?>
          </p>

          <p id="member-admin-pagination" class="pages">
...
<?php if ( bp_is_group_admin_screen( 'delete-group' ) ) : ?>
  ...
  <div id="message" class="alert">
    <p><?php _e( 'WARNING: Deleting this group will completely remove ALL content associated with it. There is no way back. Please be careful with this option.', 'qmo' ); ?></p>
  </div>
