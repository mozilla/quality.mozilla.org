...
<?php if ( post_password_required() ) : ?>
  <p class="nopassword"><?php _e( 'This post is password protected. Enter the password to view any comments.', 'qmo' ); ?></p>
</section><!-- #comments -->
<?php
...
?>
  ...
  <header class="comments-head">
    <h2><?php if($comment_count > 0) { printf(_n( 'One response', '%d responses', $comment_count, 'qmo'), $comment_count); } else { _e('No responses yet'); } ?></h2>
    <?php if (comments_open()) : ?><p class="cmt-post"><a href="#respond"><?php _e('Post a comment','qmo'); ?></a></p><?php endif; ?>
  </header>
...
<?php if ( have_comments() ) : // If there are comments ?>
  ...
  <?php if ( get_comment_pages_count() > 1 ) : // If comment paging is enabled and there are enough comments to paginate, show the comment paging ?>
    <p class="pages"><?php _e('More comments:', 'qmo'); paginate_comments_links(); ?></p>
  <?php endif; ?>
...
<?php if ( !comments_open() && pings_open() ) : // If comments are closed but pings are open ?>
  <p class="comments-closed pings-open">
    ...
    printf( __( 'Comments are closed, but <a href="%1$s" title="Trackback URL for this post">trackbacks</a> are open.', 'qmo' ), get_trackback_url() ); ?>
  </p>
<?php endif; ?>
...
<?php if (comments_open()) : ?>
  ...
  <?php if ( get_option('comment_registration') ) : // If registration is required and you're not logged in, show a message ?>
    ...
    <p><?php printf( __('You must be <a href="%s">logged in</a> to post a comment.', 'qmo'), esc_attr(wp_login_url(get_permalink())) ); ?></p>
  ...
  <?php else : // else show the form ?>
    <form id="comment-form" action="<?php echo esc_attr(get_option('siteurl')); ?>/wp-comments-post.php" method="post">
      <fieldset>
        ...
        <ol>
        <?php if ( $user_ID ) : ?>
          <li class="self"><?php printf( __( 'You are logged in as <a href="%1$s">%2$s</a>. <a class="logout" href="%3$s">Log out?</a>', 'qmo' ), admin_url( 'profile.php' ), esc_html($user_identity), wp_logout_url( apply_filters( 'the_permalink', get_permalink( $post_id ) ) ) ); ?></li>
        <?php else : ?>
          <li id="cmt-name">
            <label for="author"><?php _e('Name', 'qmo'); ?> <?php if ($req) : ?><span class="note"><?php _e('(required)', 'qmo'); ?></span><?php endif; ?></label>
          ...
          <li id="cmt-email">
            <label for="email"><?php _e('E-mail', 'qmo'); ?> <?php if ($req) : ?><span class="note"><?php _e('(required, will not be published)', 'qmo'); ?></span><?php endif; ?></label>
          ...
          <li id="cmt-web">
            <label for="url"><?php _e('Website', 'qmo'); ?></label>
            <input type="url" name="url" id="url" value="<?php echo esc_attr($comment_author_url); ?>" size="25">
          </li>
          <li id="cmt-ackbar">
            <label for="age"><?php _e('Spam robots, please fill in this field. Humans should leave it blank.', 'qmo'); ?></label>
            <input type="text" name="age" id="age" size="4" tabindex="-1">
          </li>
        <?php endif; ?>
          <li id="cmt-cmt"><label for="comment"><?php _e('Your comment', 'qmo'); ?></label> <textarea name="comment" id="comment" cols="50" rows="10"></textarea></li>
          <li id="comment-submit"><button name="submit" class="button-blue" type="submit"><?php _e('Submit Comment', 'qmo'); ?></button>
