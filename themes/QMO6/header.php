<!DOCTYPE html>
<html <?php language_attributes(); ?> id="quality-mozilla-org">
<head>
...
<?php endif; ?>
  ...
  <title><?php
    ...
    elseif ( is_search() ) { printf( __('Search results for “%s”', 'qmo'), esc_html( $s ) ); fc_page_number(); echo ' | '; bloginfo('name'); }
    elseif ( is_day() ) { $post = $posts[0]; printf(__('Posts from %s', 'qmo'), get_the_date()); echo ' | '; bloginfo('name'); fc_page_number(); }
    elseif ( is_month() ) { $post = $posts[0]; printf(__('Posts from %s', 'qmo'), get_the_date('F, Y')); echo ' | '; bloginfo('name'); fc_page_number(); }
    elseif ( is_year() ) { $post = $posts[0]; printf(__('Posts from %s', 'qmo'), get_the_date('Y')); echo ' | '; bloginfo('name'); fc_page_number(); }
...
<div id="page"><div class="wrap">
  <nav id="nav-access">
    <ul role="navigation">
      <li><a href="#content-main" tabindex="1"><?php _e( 'Skip to main content', 'qmo' ); ?></a></li>
      <li><a href="#content-sub" tabindex="2"><?php _e( 'Skip to sidebar', 'qmo' ); ?></a></li>
    <?php if ( is_active_widget( false, false, 'search', true ) || ( !is_active_sidebar('sidebar') ) ) : ?>
      <li><a href="#search" tabindex="3"><?php _e( 'Skip to blog search', 'qmo' ); ?></a></li>
    <?php endif; ?>
    </ul>
  </nav>
